  ...
  getsubcats(items) {
    this.http.get(`${RootProvider.APIURL3}itemtype2`).map(res => res.json()).subscribe(data => {
      if (data.length == null) {
        ...
        this.storage.set("subcatappdata", subcat);
    ...
    this.http.get(`${RootProvider.APIURL3}item`).map(res => res.json()).subscribe(data => {
      ...
      if (data.length == 0) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (data[i].offer_name != 'no offer') {
            this.data_with_offer.push(data[i]);
           // console.log("logging all the data without offer name = 'no offer'");
          }
        }
        console.log(data);

        console.log("ya tarek de el array");
        console.log(this.data_with_offer); 
        
        console.log(items);
        this.getsubcats(items);
        this.storage.set("appData ", items); 
        
        this.storage.set("offersData" , this.data_with_offer);
      }
    })
  }
